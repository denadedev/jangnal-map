...
모바일 홈에서는 시장별 방문 정보와 결과 시트가 접힌 상태로 시작해 지도를 넓게 보여줍니다. 결과 시트는 절반·전체 높이로 펼쳐 시장을 탐색할 수 있습니다. 시장을 선택하면 같은 시트에서 다음 장날과 방문 정보를 확인하고, 검수된 시장은 시장명을 눌러 독립 상세 페이지로 이동할 수 있습니다. 시트를 닫으면 검색 조건·결과 스크롤·포커스가 복원됩니다. 홈과 시장 상세, 온누리, 제보, 개인정보 화면은 공통 모바일 앱 바와 보조 메뉴를 사용합니다.
